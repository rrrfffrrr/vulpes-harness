...
- `openspec/CONVENTIONS.md`, `openspec/DIAGRAM-STYLE.md`, and `openspec/WRITING-STYLE.md`  (structure, diagram, and prose rules for the schemas)
...
```bash
openspec schemas                 # lists requirements, gdd, architecture, backend, frontend, persistence, ml, game-ui, verification
...
1. Replace each unit:
   ...
   - `.claude/commands/opsx/*.md`, `.codex/skills/opsx-*/`, `openspec/CONVENTIONS.md`, `openspec/DIAGRAM-STYLE.md`, `openspec/WRITING-STYLE.md`, `openspec/CLAUDE.md` - overwrite with the new files.
...
Run the workflows: `/opsx:require` -> `/opsx:gdd` or `/opsx:architect` -> `/opsx:backend` / `/opsx:frontend` / `/opsx:persistence` / `/opsx:ml` / `/opsx:game-ui` -> `/opsx:verification` -> `/opsx:propose`.
