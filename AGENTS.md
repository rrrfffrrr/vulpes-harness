...
vulpes-harness is a versioned bundle of planning assets for AI coding tools (Claude Code, Codex), layered on OpenSpec: seven workflow schemas plus the slash commands that drive them.
...
- `.claude/commands/opsx/` : Claude slash commands (require / gdd / architect / backend / frontend / game-ui / persistence)
...
- `openspec schema validate <name>` for every edited schema (`requirements` | `gdd` | `architecture` | `backend` | `frontend` | `game-ui` | `persistence`). Requires OpenSpec CLI >= 1.4.x (`npm i -g @fission-ai/openspec`).
- `npx -y markdownlint-cli2 "**/*.md"` - markdown conventions; rules and deliberate deviations live in `.markdownlint.yaml`.
- Command/skill parity - must print OK for all seven:

  ```bash
  for t in require gdd architect backend frontend game-ui persistence; do
