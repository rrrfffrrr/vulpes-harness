...
- flows/index.md (domain index) + flows/\<domain\>.md (boot flow, navigation map, modal & pause conventions, event->transition tables, statecharts)
...
- **flows** - flows/index.md is the domain INDEX only (a domain = a flow area: boot, gameplay, meta menus, settings); per-domain flows/\<domain\>.md holds its flows: the boot flow (legal placeholders -> title -> first-boot setup) in the boot domain, navigation maps jointly covering every screen including gameplay<->pause, modal stack conventions stated once in the owning domain, event->transition tables for guarded transitions, statecharts ONLY for non-trivial internal state.
